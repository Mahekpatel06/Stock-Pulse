...
public class InventoryService {
	...
	public InventoryService(TransactionRepository transactionRepository,
			InventoryRepository inventoryRepository, ProductRepository productRepository, 
			WareHouseRepository warehouseRepository, NotificationService notificationService) {
	...
	private Inventory subtractStock(UUID productId, Integer warehouseId, Integer qty) {
		...
		if (updatedInv.getQuantity() <= updatedInv.getProduct().getLow_stock_threshold()) {
			notificationService.triggerLowStockAlert(updatedInv);
		}
	...
	public Inventory saveInventory(InventoryDTO inventoryDto) {
		...
		if (saved.getQuantity() <= existingProduct.getLow_stock_threshold()) {
			notificationService.triggerLowStockAlert(saved);
		}
